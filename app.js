...
function roundToHalf(n){ return Math.round((Number(n||0))*2)/2; }
...
function ensureSettlementInvoiceDefaults(settlement, settlements = state.settlements || []){
  if (!settlement) return;
  // Invoice number is NOT pre-assigned here — it is suggested dynamically
  // and only locked when calculateSettlement() is called.
  if (!settlement.invoiceDate){
    settlement.invoiceDate = settlement.date || todayISO();
  }
}

// Pure suggestion: never writes to state.
// Returns the invoice number this settlement should get based on chronological order.
function suggestInvoiceNumber(settlement, settlements = state.settlements || []){
  if (!settlement) return null;
  if (!settlementHasInvoiceComponent(settlement)) return null;
  const candidateDate = String(settlement.invoiceDate || settlement.date || todayISO());
  const relevant = (settlements || [])
    .filter(s => s.id !== settlement.id && isSettlementCalculated(s) && s.invoiceNumber && settlementHasInvoiceComponent(s))
    .sort((a, b) => String(a.invoiceDate || "").localeCompare(String(b.invoiceDate || "")));
  const countBefore = relevant.filter(s => String(s.invoiceDate || "") <= candidateDate).length;
  return `F${countBefore + 1}`;
}

// Return the total qty from linked logs for "work" or "green", rounded to 0.5
function getTotalQtyFromLogsForKind(settlement, kind){
  const logIds = settlement.logIds || [];
  const logs = logIds.map(id => state.logs.find(l => l.id === id)).filter(Boolean);
  if (kind === "work"){
    return roundToHalf(logs.reduce((acc, log) => acc + sumWorkMs(log), 0) / 3600000);
  }
  if (kind === "green"){
    return roundToHalf(logs.reduce((acc, log) => acc + countGreenItems(log), 0));
  }
  return 0;
}

// Balanced split: pressing + on cash automatically reduces invoice (and vice versa).
// Total cash + invoice always equals what the linked logs contain.
function adjustSettlementBucketSplit(settlementId, bucket, kind, delta){
  actions.editSettlement(settlementId, (draft)=>{
    const total = getTotalQtyFromLogsForKind(draft, kind);
    const cashLine = findSettlementQuickLine(draft.lines, "cash", kind);
    const invoiceLine = findSettlementQuickLine(draft.lines, "invoice", kind);
    if (!cashLine || !invoiceLine) return;
    if (bucket === "cash"){
      const newCash = roundToHalf(Math.max(0, Math.min(total, (Number(cashLine.qty) || 0) + delta)));
      cashLine.qty = newCash;
      invoiceLine.qty = roundToHalf(Math.max(0, total - newCash));
    } else {
      const newInvoice = roundToHalf(Math.max(0, Math.min(total, (Number(invoiceLine.qty) || 0) + delta)));
      invoiceLine.qty = newInvoice;
      cashLine.qty = roundToHalf(Math.max(0, total - newInvoice));
    }
  });
}
...
function computeSettlementFromLogsInState(sourceState, customerId, logIds){
  ...
  const hours = roundToHalf(workMs / 3600000);
  ...
  for (const [productId, v] of itemMap.entries()){
    ...
    lines.push({
      ...
      unit: prod?.unit || "keer",
    qty: isGreenProduct(prod) ? roundToHalf(v.qty) : round2(v.qty),
...
const actions = {
  ...
  calculateSettlement(settlementId){
    ...
    if (settlementHasInvoiceComponent(settlement)){
      // Assign the suggested number if none was manually entered
      if (!settlement.invoiceNumber || !String(settlement.invoiceNumber).trim()){
        settlement.invoiceNumber = suggestInvoiceNumber(settlement, state.settlements || []) || getNextInvoiceNumber(state.settlements || []);
      }
...
function renderSettlementSheet(id){
  ...
  // canEdit = false once calculated — only payment status can still change
  const canEdit = !isSettlementCalculated(s);
  const isEdit = canEdit && isSettlementEditing(s.id);
  const invoiceLocked = Boolean(s.invoiceLocked || isSettlementCalculated(s));
  // Show dynamic suggestion when not yet calculated, locked number when calculated
  const invoiceNumberDisplay = isSettlementCalculated(s)
    ? String(s.invoiceNumber || "").trim().toUpperCase()
    : (String(s.invoiceNumber || "").trim().toUpperCase() || suggestInvoiceNumber(s, state.settlements) || "");
  ...
  setStatusTabbar(`
    ...
    ${canEdit ? `<button class="iconbtn" id="btnSettlementEdit" type="button" aria-label="${isEdit ? "Gereed" : "Bewerk"}" title="${isEdit ? "Gereed" : "Bewerk"}">
      ${isEdit
        ? `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12l5 5L19 7" stroke-linecap="round" stroke-linejoin="round"></path></svg>`
        : `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 21l3.5-.8L19 7.7a1.8 1.8 0 0 0 0-2.5l-.2-.2a1.8 1.8 0 0 0-2.5 0L3.8 17.5z"></path><path d="M14 5l5 5"></path></svg>`}
    </button>` : ""}
  ...
  $('#toggleCalculated')?.addEventListener('click', ()=>{
    ...
    if (!result?.ok){
      ...
      if (minDate){
        const fix = confirm(`Factuurdatum is te vroeg. Aanpassen naar ${minDate}?`);
        if (fix){
          actions.editSettlement(s.id, (draft)=>{
            draft.invoiceDate = minDate;
            draft.date = minDate;
          });
          const retry = actions.calculateSettlement(s.id);
          if (!retry?.ok) alert("Berekening mislukt, controleer de datum.");
        }
      } else {
  ...
  if (isEdit){
    ...
    $('#sheetBody').querySelectorAll('[data-logpick]').forEach(cb=>{
      cb.addEventListener('change', ()=>{
        ...
        actions.editSettlement(s.id, (draft)=>{
          ...
          // Auto-recalculate lines whenever logs change (as long as not locked)
          if (!isSettlementCalculated(draft)){
            const computed = computeSettlementFromLogsInState(state, draft.customerId, draft.logIds);
            const previousBucket = new Map((draft.lines||[]).map(li => [li.productId + "|" + li.description, li.bucket]));
            draft.lines = computed.lines.map(li => ({
              ...li,
              bucket: previousBucket.get(li.productId + "|" + li.description) || li.bucket
            }));
    ...
    $('#sheetBody').querySelectorAll('[data-settle-quick-step]').forEach(btn=>{
      ...
      bindStepButton(
        btn,
        ()=>{
          adjustSettlementBucketSplit(s.id, bucket, kind, step);
          renderSheet();
        },
        ()=>{
          adjustSettlementBucketSplit(s.id, bucket, kind, step > 0 ? 0.5 : -0.5);
          renderSheet();
        }
      );
    });
